...
RegisterServerEvent("inventory:server:GiveMoney")
AddEventHandler('inventory:server:GiveMoney', function(name, amount)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	local OtherPlayer = QBCore.Functions.GetPlayer(tonumber(name))
	...
	local Target = OtherPlayer.PlayerData.name
	local Sajat = Player.PlayerData.name
	if Player.Functions.GetItemByName("wallet") then
		if amount <= cashAmount then
			OtherPlayer.Functions.AddMoney("cash", amount)
			Player.Functions.RemoveMoney("cash", amount)
			TriggerClientEvent('QBCore:Notify', src, "Átadtál "..Target.." játékosnak "..amount.."$-t!")
			TriggerClientEvent('QBCore:Notify', name, "Kaptál "..Sajat.." játékostól "..amount.."$-t!")
		else
			TriggerClientEvent('QBCore:Notify', src, "Nincs elegendö pénzed!")
		end
	end
end)
